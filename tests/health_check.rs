...
static TRACING: Lazy<()> = Lazy::new(|| {
    ...
    // We cannot assign the output of `get_subscriber` to a variable based on the value of `TEST_LOG` because
...
/// A struct that contains information about the app being generated for testing
pub struct TestApp {
    pub address: String,
    pub db_pool: PgPool,
}
...
async fn spawn_app() -> TestApp {
    // The first time `initialize` is invoke, the code in `TRACING` is executed.
    // All other invocations will instead skip the execution
    Lazy::force(&TRACING);
    ...
    // Randomization of the database name allows for test isolation
...
/// A function to setup a new, isolate DB instance for testing
